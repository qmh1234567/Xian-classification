...
class ImageClassificationService(PTServingBaseService):
    def __init__(self, model_name, model_path):
        ...
        self.train_vec_dict = checkpoint['train_vec_dict']
    ...
    def _inference(self, data,train_set):
        ...
        with torch.no_grad():
            ...
            # pred_score = F.softmax(pred_score.data, dim=1)
            # 使用KNN决定类别
            result = Knn.KNN(train_set,pred_score,k=3)
            pred_label = self.idx_to_class[int(result)]
            result = {'result': self.label_id_name_dict[str(pred_label)]}
    ...
    def inference(self, data):
        ...
        # load data
        # train_set = np.load(os.path.join(npy_path,'best_train_set.npy')).item()
        train_set = self.train_vec_dict
        ...
        data = self._inference(data,train_set)
        infer_end_time = time.time()
        infer_in_ms = (infer_end_time - infer_start_time) * 1000
