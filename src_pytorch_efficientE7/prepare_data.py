...
def prepare_data_on_modelarts(args):
    ...
    if not args.data_url.startswith('s3://'):
        ...
        if not os.path.exists(args.data_local):
            ...
            mox.file.copy('s3://xianbucket/train_data_gen.zip', os.path.dirname(os.path.abspath(__file__))+'/train_data.zip')
            print('file copy done!')
            zip_path=os.path.dirname(os.path.abspath(__file__))
            print(mox.file.list_directory(zip_path))
            zfile = zipfile.ZipFile(os.path.dirname(os.path.abspath(__file__))+'/train_data.zip','r')           
            zfile.extractall(path=os.path.dirname(os.path.abspath(__file__)),members=zfile.namelist())
            print('解压完成！')
            print(mox.file.list_directory(zip_path))   
            train_path=os.path.dirname(os.path.abspath(__file__))+'/train_data_gen/train'
            val_path=os.path.dirname(os.path.abspath(__file__))+'/train_data_gen/val'
            print(os.path.abspath(args.data_local))
            mox.file.make_dirs(train_path)
            mox.file.make_dirs(val_path)
            mox.file.copy_parallel(train_path, args.data_local+'/train')
            mox.file.copy_parallel(val_path, args.data_local+'/val')
            print(mox.file.list_directory(args.data_local))
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', default=r'C:\Users\qmh\Desktop\\train_data\\train_data' , type=str, help='input data dir')
    parser.add_argument('--output_train_dir', default=r'\Users\qmh\Desktop\\train_data1\\train', type=str, help='output train data dir')
    parser.add_argument('--output_val_dir', default=r'\Users\qmh\Desktop\\train_data1\\val', type=str, help='output validation data dir')
