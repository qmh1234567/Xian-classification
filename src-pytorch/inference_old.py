...
class ImageClassificationService():
    def __init__(self, model_name, model_path):
        ...
        self.model = resnet_model_cbam.resnext101_32x8d()
        ...
        self.use_cuda = False
        if torch.cuda.is_available() and self.use_cuda==True:
        ...
        Resize((int(self.size*(256/224)),int(self.size*(256/224)))),
    ...
    #         pred_score = F.log_softmax(pred_score.data, dim=1)
    ...
    #     return result,str(pred_label)


    def _inference(self, data,train_set):
        img = data["input_img"]
        img = img.unsqueeze(0)
    ...
    def _predict(self,data):
        ...
        with torch.no_grad():
            ...
            # pred_vec = F.log_softmax(pred_vec.data, dim=1)
...
def plotCM(matrix, classes):
    ...
    # #         if matrix[i,j]>0:
    # #             ax.text(i, j, str('%.1f' % (matrix[i, j] * 100)), fontsize=7, va='center', ha='center',color='red')
...
def infer_on_dataset(img_dir, label_dir, model_path):
    ...
    train_set = np.load('./datasets/val_epoch_33.npy').item()


    train_set = Knn.read_data_from_npy(train_set)
    ...
    for file_name in tqdm.tqdm(files):
        ...
        try:
            ...
            result,pred = infer._inference({"input_img": img},train_set)
            y_true.append(line_split[1])  
            y_pred.append(pred)
...
if __name__ == '__main__':
    ...
    label_dir = r'./../test_20'
    model_path = r'./cache/model_snapshots/epoch_33_96.046.pth'
